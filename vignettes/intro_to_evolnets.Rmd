...
We will need four pieces of data:
...
In line with a more traditional approach of ancestral state reconstruction, we can extract the posterior probability for each interaction between ancestral parasite species and all hosts. First we need to choose which internal nodes we want to include (default is all nodes). For that, we need to look at the labels at the internal nodes of the tree file exported by RevBayes. 
...
There are 50 hosts in this data set, so `pp_at_nodes` has 50 columns. Let's have a look at a subset of those:
...
Instead of just looking at numbers, we can plot the ancestral hosts onto the parasite tree. For that, we have to choose a probability threshold over which to plot (default is 0.9). Ancestral states are colored by modules in the extant network, so first we'll define the modules.

```{r, fig.width=7, fig.height=8}
at_nodes <- posterior_at_nodes(history, tree, host_tree)

modules <- mycomputeModules(extant_net)
plot_ancestral_states(tree, at_nodes, modules)
```

We can also plot both trees, the interactions matrix, and the ancestral states at once, like so:

```{r, fig.width=10, fig.height=10}
p <- plot_module_matrix2(extant_net, at_nodes, tree, host_tree, modules)

# adjust text size
p[[2]] <- p[[2]] + theme(axis.text = element_text(size = 5))
p
```
...
Nz <- index_at_ages(samples_at_ages, ages, index = "NODF", null = 10)
```

```{r, eval=FALSE}
# calculate posterior distribution of modularity
Qz <- index_at_ages(samples_at_ages, ages, index = "Q", null = 10)
```
