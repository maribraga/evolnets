...
plot_matrix_phylo <- function(
  ...
  if (is.null(at_nodes)) {
    ...
    parasite_plot <- plot_ancestral_states(
      tree, at_nodes, modules, module_order,
...
plot_ancestral_networks <- function(
  ...
  for (n in seq_along(summary_networks)) {
    ...
    graph <- t(wnet) %>%
      ...
      dplyr::left_join(dplyr::filter(
        dplyr::select(matched_modules, .data$age, .data$name, .data$module),
        .data$age == ages[n]
      ), by = "name") %>%
  ...
  for (t in 1:length(ages)) {
    plot_age <- plot_network_at_age(
      ...
      module_levels, colors, tree,
      age = ages[t], weighted = weighted, two_state = two_state,
