...
get_z_q <- function(network, nnull = 100, use_future = FALSE) {
  ...
  # use future
  if (use_future) {
    Qrandom <- do.call(c, future.apply::future_lapply(1:nnull, function(x) {
      mycomputeModules(null_nets[, , x])@likelihood
    }, future.seed = T))
  ...
  for (a in seq_along(sampled_networks)) {
    for (i in seq_len(dim(sampled_networks[[a]])[1])) {
      ...
      if (ncol(net) < 2 || nrow(net) < 2 || is.vector(net)) {
        ...
        if (use_future) {
          Qrandom <- do.call(c, future.apply::future_lapply(1:nnull, function(x) {
            mycomputeModules(sim[, , x])@likelihood
          }, future.seed = T))
          for (j in 1:nnull) {
