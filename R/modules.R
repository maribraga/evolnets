    ...
    for (i in seq_along(no_split)) {
      ...
      if (length(mod_no_split) == 0) {
        ...
        for (c in seq_along(strength_no_split)) {
          mod_df_sym_age[
            which(mod_df_sym_age$name == no_split[i]),
            names(strength_no_split)[c]
          ] <- strength_no_split[names(strength_no_split)[c]]
        }
      }
    }
  }
    ...
    if (n_originals > 1) {
      ...
      for (o in seq_along(originals)) {
        ...
        if (mods_left[m] %in% sub_mod_left_updated) {
          ...
          mod_df_sym_age[
            which(mod_df_sym_age$module_name == mods_left[m] &
              mod_df_sym_age$original_module == originals[o]),
            "module_name"
          ] <- paste0(sub_let, ".", which(originals == originals[o]) + nsubs)
        } else {
          mod_df_sym_age[
            which(mod_df_sym_age$module_name == mods_left[m] &
              mod_df_sym_age$original_module == originals[o]),
            "module_name"
          ] <- paste0(mods_left[m], ".", which(originals == originals[o]))
        }
      }
    }
  }
...
process_child_modules <- function(mod_df_sym, age_min, children, nodes_interval, sym_mod_el, mods_strength, node_treeio, child_idx) {
  ...
  if (children[child_idx] %in% dplyr::filter(mod_df_sym, .data$age == age_min)$name) {
    ...
    if (length(mod_child) == 0) {
      nodes_interval[
        which(nodes_interval$node == node_treeio),
        child_str
      ] <- NA
    } else {
      nodes_interval[
    ...
    for (n in seq_len(nrow(node_depth))) {
      ...
      if (!is.null(children_strengths)) {
        ...
        for (c in seq_along(mean_strengths)) {
          nodes_interval[
            which(nodes_interval$node == node_treeio),
            names(mean_strengths)[c]
          ] <- mean_strengths[names(mean_strengths)[c]]
        }
        ...
        nodes_interval[
          which(nodes_interval$node == node_treeio),
          "module_name"
        ] <- sub("strength_", "", names(mean_strengths)[strongest_mod_idx])
        nodes_interval[
          which(nodes_interval$node == node_treeio),
          "strength"
        ] <- mean_strengths[strongest_mod_idx]

        # Write strengths for nodes in the network at max age
        if (node_name_treeio %in% mod_df_sym_age$name) {
          for (c in seq_along(mean_strengths)) {
            mod_df_sym_age[
              which(mod_df_sym_age$name == node_name_treeio),
              names(mean_strengths)[c]
            ] <- mean_strengths[names(mean_strengths)[c]]
          }
...
remove_duplicate_modules <- function(mod_samples) {
  duplicates_removed <- mod_samples %>%
    ...
    dplyr::mutate(original_module = dplyr::case_when(
      .data$problem == "YES" ~ 1,
